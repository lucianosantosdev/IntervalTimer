...
- `:wear` — Android **application**, Wear Compose + `wear-ongoing` for ongoing activity notifications. **No Koin, no DI module** — settings are instantiated directly (e.g. `TimerSettingsRepositoryImpl(LocalContext.current)` in `MobileNavHost` / wear equivalent). Do not assume DI is wired when editing wear code.
